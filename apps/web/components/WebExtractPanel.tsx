...
export function WebExtractPanel() {
  ...
  const handleSubmit = async () => {
    ...
    for (const task of pending) {
      ...
      setTasks((prev) => prev.map((t) => (t.id === task.id ? { ...t, status: "extracting", progress: "上传并提取中..." } : t)));
      ...
      try {
        ...
        setTasks((prev) => prev.map((t) => (t.id === task.id ? { ...t, status: "done", audioUrl, audioName, progress: "" } : t)));
      } catch (err) {
        if (controller.signal.aborted) break;
        ...
        {tasks.length === 0 ? (
          ...
          >
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <div>
                <p className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>点击选择视频文件</p>
                <p className="text-xs mt-1" style={{ color: "var(--color-text-tertiary)" }}>从视频中提取音频为 WAV 格式</p>
              </div>
          ...
          <div className="flex flex-col gap-1.5">
            <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>视频文件</p>
            {tasks.map((t, i) => (
              <div key={t.id} className="flex items-center gap-2 text-xs px-2 py-1 rounded"
                ...
                <button
                  onClick={() => { URL.revokeObjectURL(t.audioUrl ?? ""); setTasks((prev) => prev.filter((x) => x.id !== t.id)); }}
        ...
        <div className="flex gap-2">
          {isRunning ? (
            ...
            <button onClick={handleSubmit}
              ...
              {tasks.length === 0 ? "请先选择视频文件" : `开始提取（${tasks.filter((t) => t.status === "pending" || t.status === "error").length} 个文件）`}
            </button>
          )}
        </div>
      </div>
            ...
            {tasks.map((t, i) => (
              <div key={t.id} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {t.status === "extracting" && <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>{t.progress}</p>}
                  {t.status === "error" && <p className="text-xs" style={{ color: "var(--color-danger)" }}>{t.error}</p>}
                  {t.status === "done" && t.audioName && (
                    <p className="text-xs" style={{ color: "var(--color-accent)" }}>↓ {t.audioName}</p>
                  )}
                </div>
                {t.status === "done" && t.audioUrl && (
                  <button onClick={() => triggerDownload(t.audioUrl!, t.audioName!)}
