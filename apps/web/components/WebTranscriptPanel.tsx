...
export function WebTranscriptPanel() {
  ...
  return (
    <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-2">
          {isRunning ? (
            ...
            >
              {tasks.length === 0
                ? "请先选择音频文件"
                : `开始转录（${tasks.filter((t) => t.status === "pending" || t.status === "error").length} 个文件）`}
            </button>
          )}
        </div>
      </div>
      ...
      {tasks.some(
        (t) => t.status === "uploading" || t.status === "processing" || t.status === "done" || t.status === "error"
      ) && (
        <div
              ...
              <div key={t.id} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {t.status === "done" && t.segments && (
                    <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>
                      {t.segments.length} 条字幕
                    </p>
                  )}
                </div>
                {t.status === "done" && t.segments && (
                  <button
                    onClick={() => downloadSrt(t.file.name, t.segments!)}
