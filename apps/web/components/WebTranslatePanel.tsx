...
function parseSrt(content: string): Segment[] {
  ...
  for (const block of blocks) {
    ...
    const start =
      parseInt(timeMatch[1]) * 3600 + parseInt(timeMatch[2]) * 60 + parseInt(timeMatch[3]) + parseInt(timeMatch[4]) / 1000;
    const end =
      parseInt(timeMatch[5]) * 3600 + parseInt(timeMatch[6]) * 60 + parseInt(timeMatch[7]) + parseInt(timeMatch[8]) / 1000;
...
export function WebTranslatePanel() {
  ...
  const [results, setResults] = useState<{ name: string; originalSrt: string; translatedSrt: string; bilingualSrt: string | null }[]>([]);
    ...
    <div className="flex flex-col gap-3">
      ...
      >
          ...
          <div className="flex flex-col gap-1 max-h-32 overflow-y-auto">
            {files.map((f, i) => (
              <div key={i} className="flex items-center justify-between text-xs px-2 py-1 rounded"
                ...
                <span className="truncate flex-1" style={{ color: "var(--color-text-secondary)" }}>{f.name}</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          <LanguageSelect label="源语言" value={sourceLang} onChange={setSourceLang} options={SOURCE_LANGUAGES} disabled={status === "processing"} />
          <LanguageSelect label="目标语言" value={targetLang} onChange={setTargetLang} options={TARGET_LANGUAGES} disabled={status === "processing"} />
        </div>

        <label className="flex items-center gap-3 cursor-pointer group">
          <div className="relative">
            <input type="checkbox" className="sr-only" checked={bilingual} onChange={(e) => setBilingual(e.target.checked)} disabled={status === "processing"} />
            <div className="w-9 h-5 rounded-full transition-all duration-200" style={{ background: bilingual ? "var(--color-accent)" : "var(--color-surface-3)" }} />
            <div className="absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-all duration-200 shadow-sm"
              style={{ transform: bilingual ? "translateX(16px)" : "translateX(0)" }} />
          </div>
          <span className="text-sm select-none" style={{ color: "var(--color-text-secondary)" }}>生成双语字幕</span>
        </label>
        ...
        >
          {status === "processing" ? `翻译中 (${progress}/${total})...` : files.length === 0 ? "请先选择 SRT 文件" : `开始翻译（${files.length} 个文件）`}
        </button>

        {status === "error" && error && (
          <div className="rounded-md px-4 py-3 text-sm" style={{ background: "oklch(65% 0.20 20 / 8%)", color: "var(--color-danger)" }}>
            <pre className="text-xs whitespace-pre-wrap" style={{ fontFamily: "JetBrains Mono, monospace" }}>{error}</pre>
          </div>
        )}
      </div>
      ...
      {status === "done" && results.length > 0 && (
        <div className="rounded-md overflow-hidden" style={{ background: "var(--color-surface-1)", border: "0.5px solid var(--color-border-subtle)" }}>
          <div className="px-4 py-2.5 flex items-center justify-between" style={{ borderBottom: "0.5px solid var(--color-border-subtle)" }}>
            <span className="text-sm font-medium" style={{ color: "var(--color-text-primary)" }}>翻译结果</span>
            <span className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>{results.length} 个文件</span>
          </div>
            ...
            {results.map((r, i) => (
              <div key={i} className="px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex gap-1.5 shrink-0">
                  <button onClick={() => downloadBlob(r.translatedSrt, r.name.replace(/\.srt$/i, ".translated.srt"))}
                    className="px-2.5 py-1 text-xs rounded"
                    style={{ background: "var(--color-accent)", color: "white" }}>
                    下载译文
                  </button>
                  {r.bilingualSrt && (
                    <button onClick={() => downloadBlob(r.bilingualSrt, r.name.replace(/\.srt$/i, ".bilingual.srt"))}
                      className="px-2.5 py-1 text-xs rounded"
