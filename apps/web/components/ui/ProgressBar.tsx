...
export function ProgressBar({ step, taskProgress, uploadLabel, filename }: ProgressBarProps) {
  ...
  const stageLabel = isProcessing ? (STAGE_LABEL[taskProgress.stage] ?? taskProgress.stage) : ""
  const stageColor = isProcessing ? (STAGE_COLOR[taskProgress.stage] ?? "var(--color-accent)") : "var(--color-accent)"
        ...
        return (
          <div key={s.key} className="flex items-center flex-1 last:flex-none">
            ...
            {i < FLOW_STEPS.length - 1 && (
              <div
                className="flex-1 h-px mx-1.5 rounded-full transition-all duration-500"
                style={{
  ...
  if (isCompressing) {
    statusLine = (
      <div className="flex items-center gap-2 text-xs" style={{ color: "var(--color-text-secondary)" }}>
        <span
          className="inline-block w-3 h-3 border-2 rounded-full border-t-transparent animate-spin"
