...
export function BatchProgress({ files, currentIndex }: BatchProgressProps) {
  ...
  // 当前活跃文件（正在处理中的）
  const activeFiles = files.filter((f) => f.status !== "pending" && f.status !== "done")
    ...
    >
        ...
        <div className="flex gap-2 text-xs">
          {doneCount > 0 && (
            <span
              ...
              style={{ background: "oklch(50% 0.15 145 / 15%)", color: "oklch(65% 0.15 145)" }}
            >
              ✓ 已完成 {doneCount}
            </span>
          )}
          {errorCount > 0 && (
            <span
              className="px-2 py-0.5 rounded-full"
              style={{ background: "oklch(50% 0.20 20 / 15%)", color: "var(--color-danger)" }}
            >
              ✗ 失败 {errorCount}
            </span>
          )}
          {inProgressFiles.length > 0 && (
            <span
              className="px-2 py-0.5 rounded-full"
              style={{ background: "oklch(50% 0.20 280 / 15%)", color: "oklch(70% 0.20 280)" }}
            >
      ...
      <div className="divide-y" style={{ borderColor: "var(--color-border-subtle)" }}>
        {files.map((f) => {
          ...
          const shortName =
            f.file.name.length > 36 ? f.file.name.slice(0, 16) + "…" + f.file.name.slice(-16) : f.file.name
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {isActive ? (
                    ...
                    <span className="shrink-0 text-xs" style={{ color: badge.text }}>
                      ✓
                    </span>
                  ) : f.status === "error" ? (
                    <span className="shrink-0 text-xs" style={{ color: badge.text }}>
