...
export function ExtractPanel() {
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event")
      .then(({ listen }) =>
        listen<ExtractProgress>("extract-progress", (e) => {
          ...
          targetProgressRef.current = (() => {
            const arr =
              targetProgressRef.current.length === total ? [...targetProgressRef.current] : new Array(total).fill(0)
            arr[index] = ratio
            return arr
          })()
        })
  ...
  const addPaths = useCallback(async (newPaths: string[]) => {
    ...
    setInputs((prev) => {
      ...
      setInputSizes((prevSizes) => [...prevSizes, ...toAdd.map((p) => sizes[newPaths.indexOf(p)] ?? 0)])
      return [...prev, ...toAdd]
    })
  }, [])
