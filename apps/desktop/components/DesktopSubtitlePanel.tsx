...
function settingsComplete(s: Settings, modelReady: boolean, downloadedModels?: Set<WhisperModel>): boolean {
  const asrOk =
    s.asrProvider === "local-whisper"
      ? downloadedModels
        ? downloadedModels.has(s.whisperModel)
        : modelReady
      : s.asrProvider === "groq"
        ? !!s.groqApiKey.trim()
        : !!s.siliconflowApiKey.trim()
  const trlOk =
    s.translateProvider === "tencent"
    ...
    <div ref={ref} className="relative w-full">
      ...
      >
        ...
        <span className="relative flex items-center gap-2">
          <span style={{ color: downloadingInfo && !open ? "var(--color-text-primary)" : "var(--color-text-primary)" }}>
            {selected?.label}
          </span>
...
export function DesktopSubtitlePanel() {
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/core").then(({ invoke }) => {
      invoke<{
        ...
        .catch(() => null)
      invoke<{ concurrency: number; gpu: boolean; vram_mb: number | null; gpu_name: string }>("get_concurrency", {
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event")
      .then(({ listen }) =>
        listen<{ variant: string; ratio: number; message: string; url: string }>("gpu-download-progress", (e) => {
    ...
    <div className="flex flex-col gap-3 max-w-3xl mx-auto w-full">
      ...
      >
        ...
        <div className="flex gap-2">
          ...
          >
            {isRunning
              ? `生成中 (${tasks.filter((t) => t.status === "done").length}/${tasks.length})...`
              : "开始生成字幕"}
          </button>
          {isRunning && (
            <button
                ...
                <div key={task.path} style={{ borderColor: "var(--color-border-subtle)" }}>
                  ...
                  {task.status === "error" && isExpanded && (
                    ...
                    >
                      ...
                      ) : task.error.includes("网络") ||
                        task.error.includes("network") ||
                        task.error.includes("fetch") ? (
                        <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>
                          提示：请检查网络连接后重试
                        </p>
                      ) : null}
                          ...
                          {PIPELINE_STEPS.filter((s) => task.stageTiming[s.key] != null).map((s) => (
                            ...
                            >
                              ...
                              <span style={{ color: "var(--color-accent)" }}>
                                {task.stageTiming[s.key]!.toFixed(1)}s
                              </span>
                            </span>
                          ))}
                        </div>
              ...
              {settings.asrProvider === "groq" && (
                <SettingRow label="API Key">
                  <div className="flex flex-col gap-1">
                    ...
                    />
                    <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>
                      在 <span style={{ color: "var(--color-accent)" }}>console.groq.com</span> 免费获取，每天 14400
                      分钟配额
                    </p>
                  </div>
                </SettingRow>
