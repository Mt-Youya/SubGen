      ...
      >
        {downloadingInfo && !open && (
          <div
            ...
            style={{
              transform: `scaleX(${downloadingPct / 100})`,
              ...
              <div key={m.name} className="relative">
                {isDownloading && (
                  <div
                    ...
                    style={{ width: `${pct}%`, background: "var(--color-accent-muted)" }}
                  />
                )}
                      ...
                      <>
                        ...
                        >
                          <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                            <path
                              d="M2 3h8M5 3V2h2v1M4.5 3l.5 6h2l.5-6"
                              stroke="currentColor"
                              strokeWidth="1.2"
                              strokeLinecap="round"
                              strokeLinejoin="round"
...
export function TranscriptPanel() {
  ...
  useEffect(() => {
    whisperModelRef.current = settings.whisperModel
  ...
  const checkModel = useCallback(async () => {
    ...
    }>("check_whisper_model", { model: whisperModelRef.current }).catch(() => ({
      whisper: false,
      model: false,
      models: [],
    }))
    ...
    if (saved) {
      try {
        ...
        setSettings({
          ...
          siliconflowApiKey: parsed.siliconflowApiKey ?? "",
          chunkSeconds: parsed.chunkSeconds ?? 240,
  ...
  useEffect(() => {
    ...
    import("@tauri-apps/api/event")
      .then(({ listen }) =>
        listen<{ model: string; ratio: number }>("model-download-progress", (e) => {
  ...
  const canSubmit = useMemo(
    () =>
      ...
      !!outputDir &&
      !isRunning &&
      settingsComplete(settings, modelReady, downloadedModels),
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setQueueElapsed((Date.now() - queueStart) / 1000)
    setQueueStartTime(null)
  ...
  const handleStop = useCallback(() => {
    ...
    import("@tauri-apps/api/core").then(({ invoke }) => {
      setTasks((prev) => {
        for (const t of prev) {
          if (t.status === "processing") invoke("cancel_subtitle", { input: t.path }).catch(() => {})
        }
        return prev
      })
    })
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      {task.status === "error" && !isRunning && (
                        <button
                          onClick={() =>
                            setTasks((prev) =>
                              prev.map((t) =>
                                t.path === task.path ? { ...t, status: "pending", error: "", result: null } : t
                              )
                            )
                          }
                          className="text-xs px-2 py-0.5 rounded"
                      ...
                      {task.status === "pending" && !isRunning && (
                        <button
                          onClick={() => setTasks((prev) => prev.filter((t) => t.path !== task.path))}
                          className="text-xs px-2 py-0.5 rounded"
                          ...
                          >
                            <div className="flex items-center pt-2.5">
                              {PIPELINE_STEPS.map((s, i) => {
                                ...
                                const stepElapsed = (() => {
                                  if (i === 0)
                                    return task.stageTiming["extracting"] ?? task.stageTiming["loading_model"]
                      ...
                      {Object.keys(task.stageTiming).length > 0 && (
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 pt-2">
                          {PIPELINE_STEPS.filter((s) => {
                            ...
                            const elapsed =
                              s.key === "extracting" ? task.stageTiming["extracting"] : task.stageTiming[s.key]
                            return (
                              <span
                                key={s.key}
                  ...
                  {gpuStatus.active_is_gpu ? (
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="rounded px-2 py-0.5 text-xs"
                        style={{ border: "0.5px solid var(--color-border)", color: "var(--color-text-tertiary)" }}
                      >
                        清除
                      </button>
                    </div>
              ...
              {settings.asrProvider === "groq" && (
                <SettingRow label="API Key">
                  <div className="flex flex-col gap-1">
                    ...
                    />
                    <p className="text-xs" style={{ color: "var(--color-text-tertiary)" }}>
                      在 <span style={{ color: "var(--color-accent)" }}>console.groq.com</span> 免费获取
                    </p>
                  </div>
                </SettingRow>
              )}
