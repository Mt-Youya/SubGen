...
## 4. Update skills and README

Run `git diff --name-only HEAD` to see which files changed.

- If any file under `packages/desktop/` or `packages/web/` changed, update `CLAUDE.md` (at repo root) to reflect any new key paths, commands, or dependencies introduced.
- If UI components or features were added/changed, append a one-line summary to the relevant section in `README.md` (if it exists).

Skip silently if nothing relevant changed.
